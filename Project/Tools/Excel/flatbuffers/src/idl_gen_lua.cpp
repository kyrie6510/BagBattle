...
const char *SelfKeysIndex = "self.KeysIndex";
...
class LuaGenerator : public BaseGenerator {
 public:
  ...
  void InitializeExisting(const StructDef &struct_def, std::string *code_ptr) {
    std::string &code = *code_ptr;
    std::string structname = struct_def.name;
    unsigned long indx = structname.find("RowData");
    GenReceiver(struct_def, code_ptr);
    code += "Init(buf, pos)\n";
    code +=
        std::string(Indent) + SelfData + " = flatbuffers.view.New(buf, pos)\n";
    if (parser_.opts.lua_gen_KeysIndex && indx == std::string::npos)
    {
      code +=
          std::string(Indent) + SelfKeysIndex + " = {}\n";
      code +=
          std::string(Indent) + "local len = self:DatalistLength()\n";
      code +=
          std::string(Indent) + "for i=1, len  do\n";
      code += std::string(Indent) + Indent + "local curdata = self:Datalist(i)\n";
      code += std::string(Indent) + Indent + "local curid = curdata:Id()\n";
      code += std::string(Indent) + Indent + "self.KeysIndex[curid] = i\n";
  ...
  void GetMemberOfVectorOfStruct(const StructDef &struct_def,
    ...
    if (parser_.opts.lua_gen_KeysIndex) 
    {
      GenReceiver(struct_def, code_ptr);
      code += MakeCamel(NormalizedName(field));
      code += "Id(j)\n";
      code +=
          std::string(Indent) + "return self:Datalist(self.KeysIndex[j]);\n";
      code += EndFunc;
    }
    GenReceiver(struct_def, code_ptr);
    code += MakeCamel(NormalizedName(field));
    code += "(j)\n";
    code += std::string(Indent) + "if not j then return end\n";
    code += OffsetPrefix(field);
    code +=
